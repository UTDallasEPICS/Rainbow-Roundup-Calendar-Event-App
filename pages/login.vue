...
<template>
  <div class="w-full min-h-screen flex items-center justify-center bg-white px-6 py-12">
    ...
    <form @submit.prevent="submitEmail" class="w-full max-w-xl rounded-2xl p-8 sm:p-10 flex flex-col items-center"
      ...
      <div class="w-full">
        ...
        <input v-model="email" type="email" placeholder="you@example.com"
          class="w-full px-4 py-3 text-lg rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-rr-sky transition" />
      </div>

      <!-- Send Magic Link Button -->
      <button type="submit"
        class="mt-8 w-full sm:w-[280px] py-4 px-6 text-xl text-white font-semibold bg-rr-magenta hover:bg-rr-magentaDark transition rounded-2xl">
        Send Login Code
      </button>
      ...
      <div class="text-lg mt-6 text-center text-gray-600">
        <strong>
          ...
          <NuxtLink to="/signup" class="text-rr-magenta hover:underline transition">Sign Up</NuxtLink>
        </strong>
      </div>
    </form>
    ...
    <h2 class="text-2xl sm:text-3xl font-extrabold text-center text-rr-navy mb-8" v-if="formMode==='done'">
        {{ responseMessage }}
        <div>
