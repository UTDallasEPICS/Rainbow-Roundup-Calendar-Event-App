    ...
    <form @submit.prevent="submitSignupForm"
      ...
      <div class="space-y-6 w-full">
        ...
        <div>
          ...
          <input type="text" v-model="signupModel.firstname"
            class="w-full px-4 py-3 text-md rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-colour_sky transition" />
        </div>
      ...
      <div class="text-md mt-6 text-center text-gray-600">
        <strong>
          ...
          <NuxtLink to="/login" class="text-colour_magenta hover:underline transition">
            Sign In
          </NuxtLink>
        </strong>
      </div>
      ...
      <!-- Register Button -->
      <button type="submit"
        class="mt-8 w-full sm:w-[300px] py-4 px-6 text-xl text-white font-semibold bg-colour_magenta hover:bg-colour_magentaDark transition rounded-2xl">
        Register
      </button>
      <!-- Success message -->
