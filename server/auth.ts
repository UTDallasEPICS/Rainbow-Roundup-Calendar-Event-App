...
export const auth = betterAuth({
  ...
  session: {
    // add a signed 5 minute cookie to user, if it exists and is valid, return session with fewer db calls
    cookieCache: {
      // this is added because loading session can be slow even on local connection, adding this makes it a bit faster
  ...
  callbacks: {
    async signIn({ user, account, email }) {
      // todo: understand if account and email need ot be cut
    ...
    emailOTP({
      async sendVerificationOTP({ email, otp, type }) {
        const user = await prisma.user.findUnique({
          // we don't want people banned/archived users to login
          where: { email: email },
        });
        if (type === "sign-in") {
          if (!user) {
